...
class Varios
{
        ...
        switch ($algoritmo) {
            ...
            case 'ssha':
                if (function_exists('hash')) {
                    ...
                    // $password = base64_encode(pack("H*"hash('sha1', $passwordPlano.$salt, true).$salt);
                    $password = base64_encode(pack("H*",sha1($passwordPlano.$salt)).$salt);
                } else {
    ...
    /**
	 * Implementacion de mhash_keygen_s2k en PHP, se evita usar la extension
	 * @param string $hash
	 ...
	 */
	private function keygen_s2k($hash, $password, $salt, $bytes)
	{
		$result = false;
		if (function_exists('hash')) {
			$count = ceil($bytes / strlen(hash($hash, null, true))) - 1;
			for ($i=0; $i <= $count; $i++) {
